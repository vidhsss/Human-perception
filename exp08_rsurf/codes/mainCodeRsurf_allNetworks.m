...
mean_data=zeros(N,123);
sem_data=zeros(N,123);
for ind=1:1
    mean_data(ind,:)=MI_across_layers{sel_index(ind),1};
    sem_data(ind,:)=MI_across_layers{sel_index(ind),2};
end
y_label='average of surface invariance index for top 9 % tetrads 2';
